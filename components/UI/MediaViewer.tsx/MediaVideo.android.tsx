...
function MediaVideoContent(
  ...
  useEffect(() => {
    ...
    if (focused) {
      ...
      // feed player's "mixWithOthers" muted state. Muted state follows the
      // tapped-video audio setting (the mute control in this viewer's own
      // controls row), which defaults to the feed's audio setting until the
      // user sets it.
      player.audioMixingMode = "doNotMix";
        ...
        >
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              // (its docs say true), so a >1x rate shifts the pitch up and
              // turns voices into chipmunks. Force it on with every rate change.
