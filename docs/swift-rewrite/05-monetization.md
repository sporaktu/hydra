...
| M | Navigation / gestures (structural) | Tab bar, per-tab stacks, tap-active-tab-to-pop, edge-swipe back, "swipe anywhere to navigate", long-press menus, deep links, **the iPad split view in full** — the two-pane layout, its Appearance toggle, its Close and Fullscreen controls, and every interaction inside the pane | — | Structural navigation must never be gated, and the split view **is** navigation: it is the shape of the screen on an iPad, exactly as the tab bar is the shape of the app. Gating it would leave an iPad user with a layout that looks broken rather than a feature they have not bought, and the owner rates it as very important. **No gate id exists for it, and none may be added** (`[DECISION: ipad-split-view-in-scope]`). |
